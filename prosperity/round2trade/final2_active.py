...
# Swing-reversal signal + decaying target boost
ASH_SWING_RETRACE = 2.5
ASH_SWING_MIN_RANGE = 6.0
ASH_BOOST_DURATION_MS = 3000
ASH_BURST_TAKE_DISTANCE = 3.0
...
def _ash_burst_sweep_buy(order_depth, q: int, inner_fair: float) -> tuple[list[Order], int]:
    orders: list[Order] = []
    buy_cap = max(ASH_Q_MAX - q, 0)
    if buy_cap <= 0:
        return orders, q
    upper = inner_fair + ASH_BURST_TAKE_DISTANCE
    for ask_px in sorted(order_depth.sell_orders.keys()):
        if ask_px >= upper or buy_cap <= 0:
            break
        avail = max(-int(order_depth.sell_orders[ask_px]), 0)
        take = min(avail, buy_cap)
        if take > 0:
            orders.append(Order(ASH_TARGET, int(ask_px), int(take)))
            q += take
            buy_cap -= take
    return orders, q


def _ash_burst_sweep_sell(order_depth, q: int, inner_fair: float) -> tuple[list[Order], int]:
    orders: list[Order] = []
    sell_cap = max(ASH_Q_MAX + q, 0)
    if sell_cap <= 0:
        return orders, q
    lower = inner_fair - ASH_BURST_TAKE_DISTANCE
    for bid_px in sorted(order_depth.buy_orders.keys(), reverse=True):
        if bid_px <= lower or sell_cap <= 0:
            break
        avail = max(int(order_depth.buy_orders[bid_px]), 0)
        take = min(avail, sell_cap)
        if take > 0:
            orders.append(Order(ASH_TARGET, int(bid_px), -int(take)))
            q -= take
            sell_cap -= take
    return orders, q


def _ash_swing_signals(memory: dict, inner_fair: float) -> tuple[bool, bool]:
    """State machine tracking the most recent peak/trough.

    Emits peak_reversal (just confirmed a peak: price dropped RETRACE from it)
    or trough_reversal (just confirmed a trough: price rose RETRACE from it),
    gated by MIN_RANGE between the newly confirmed extreme and the prior one.
    """
    swing_state = memory.get("swing_state", "init")
    extreme = float(memory.get("swing_extreme", inner_fair))
    last_opposite = float(memory.get("swing_last_opposite", inner_fair))
    ...
    if swing_state == "init":
        swing_state = "up"
        extreme = inner_fair
        last_opposite = inner_fair
    elif swing_state == "up":
        if inner_fair > extreme:
            extreme = inner_fair
        elif extreme - inner_fair >= ASH_SWING_RETRACE:
            if extreme - last_opposite >= ASH_SWING_MIN_RANGE:
                peak_reversal = True
            swing_state = "down"
            last_opposite = extreme
            extreme = inner_fair
    elif swing_state == "down":
        if inner_fair < extreme:
            extreme = inner_fair
        elif inner_fair - extreme >= ASH_SWING_RETRACE:
            if last_opposite - extreme >= ASH_SWING_MIN_RANGE:
                trough_reversal = True
            swing_state = "up"
            last_opposite = extreme
            extreme = inner_fair

    memory["swing_state"] = swing_state
    memory["swing_extreme"] = extreme
    memory["swing_last_opposite"] = last_opposite
    return peak_reversal, trough_reversal
...
def _run_ash_strategy(state: TradingState, memory: dict) -> list[Order]:
    ...
    peak_reversal, trough_reversal = _ash_swing_signals(memory, inner_fair)

    boost_dir = int(memory.get("boost_dir", 0))
    boost_expire_ts = int(memory.get("boost_expire_ts", 0))

    if ts >= boost_expire_ts:
        boost_dir = 0

    burst_orders: list[Order] = []
    if trough_reversal:
        sweep, q = _ash_burst_sweep_buy(order_depth, q, inner_fair)
        burst_orders.extend(sweep)
        boost_dir = 1
        boost_expire_ts = ts + ASH_BOOST_DURATION_MS
    elif peak_reversal:
        sweep, q = _ash_burst_sweep_sell(order_depth, q, inner_fair)
        burst_orders.extend(sweep)
        boost_dir = -1
        boost_expire_ts = ts + ASH_BOOST_DURATION_MS

    memory["boost_dir"] = boost_dir
    memory["boost_expire_ts"] = boost_expire_ts

    q_star_base, _ = _ash_target_position(inner_fair)
    if boost_dir != 0:
        q_star = float(boost_dir * ASH_Q_MAX)
    else:
        q_star = q_star_base
    delta_q = q_star - q
    ...
    return burst_orders + take_orders + outer_orders + inner_orders
