...
def _zeroing_take(order_depth, q: int, inner_fair: float) -> tuple[list[Order], int]:
    orders: list[Order] = []
    ...
    if q < 0:
        ...
        for ask_px in sorted(order_depth.sell_orders.keys()):
            if ask_px >= inner_fair or remaining <= 0:
                break
            avail = max(-int(order_depth.sell_orders[ask_px]), 0)
            if avail <= 0:
                continue
            take_size = min(avail, remaining)
        ...
        for bid_px in sorted(order_depth.buy_orders.keys(), reverse=True):
            if bid_px <= inner_fair or remaining <= 0:
                break
...
def _build_quote_prices(order_depth, inner_fair: float, memory: dict) -> tuple[int | None, int | None, int | None, int | None]:
    ...
    return my_bid, my_ask, best_bid_opp, best_ask_opp


def _dual_passive_quote(order_depth, q: int, inner_fair: float, memory: dict) -> list[Order]:
    orders: list[Order] = []
    my_bid, my_ask, _, _ = _build_quote_prices(order_depth, inner_fair, memory)
...
class Trader:
    def run(self, state: TradingState):
        ...
        take_orders, q_after_take = _zeroing_take(order_depth, q, inner_fair)
