"""round4follow.py — round3final.py + 跟单层 + 仓位分档
...
仓位分档（v2 新增）：
  - 弱信号区：entry ≤ |dev| < strong  → 目标仓位 = sign · WEAK_FRAC · limit
  - 强信号区：|dev| ≥ strong          → 目标仓位 = sign · limit
  开仓量用 target − position（带符号），保留预算给真正的极端信号。
"""
...
# ---- 仓位分档（v2 新增） -------------------------------------------------
# 弱信号占多大比例的仓位上限。强信号一律 1.0（打满）。
WEAK_FRAC: Dict[str, float] = {p: 0.8 for p in PRODUCTS}

# 强信号门槛 |dev|。
# - PASSIVE_QUOTE / TAKER_NEAR 类：用 ACTIVE_THRESHOLD 的 0.9 倍作为强信号线。
# - TAKER_ONLY 类：强信号取 TAKER_ONLY_THRESHOLD 的 1.3 倍。
STRONG_THRESHOLD: Dict[str, float] = {}
for _p in PRODUCTS:
    if _p in TAKER_ONLY:
        STRONG_THRESHOLD[_p] = TAKER_ONLY_THRESHOLD[_p] * 1.3
    else:
        STRONG_THRESHOLD[_p] = ACTIVE_THRESHOLD[_p] * 0.9
...
# v2：从 1.0 降到 0.3，让跟单只是加成而不是顶仓。
FOLLOW_MAX_FRAC: Dict[str, float] = {p: 0.3 for p in PRODUCTS}
...
def _target_position(product: str, deviation: float, limit: int) -> int:
    """两档目标仓位（带符号）。
    deviation < 0 → 价格低于 EMA → 反向开多 → 目标 > 0
    deviation > 0 → 价格高于 EMA → 反向开空 → 目标 < 0
    """
    abs_dev = abs(deviation)
    sign = 1 if deviation < 0 else -1
    if abs_dev >= STRONG_THRESHOLD[product]:
        return sign * limit
    return sign * int(round(limit * WEAK_FRAC[product]))
    ...
    def run(self, state: TradingState):
        ...
        for product in PRODUCTS:
            ...
            if mark and source:
                ...
                if net != 0 and self._ema_in_trade_zone(product, deviation):
                    # only follow when EMA direction agrees with mark's net
                    if (net > 0 and deviation < 0) or (net < 0 and deviation > 0):
                        # how many seats are still free up to the *tier target*?
                        ema_signed = sum(o.quantity for o in orders)
                        proj_pos = position + ema_signed
                        target = _target_position(product, deviation, limit)
                        max_qty = int(round(limit * FOLLOW_MAX_FRAC[product]))
                        if net > 0:
                            # 跟买：仅允许把仓位补到 target（可能是 0.8·limit）
                            room = max(0, target - proj_pos)
